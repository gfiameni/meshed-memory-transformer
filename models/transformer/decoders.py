...
class Decoder(Module):
    def __init__(self, vocab_size, max_len, N, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1,
                 pe_every_layer=False, self_att_module=None, enc_att_module=None, self_att_module_kwargs=None,
                 enc_att_module_kwargs=None):
        ...
        self.pos_emb = nn.Embedding.from_pretrained(sinusoid_encoding_table(max_len + 1, d_model, 0),
                                                    freeze=True)
        self.layers = ModuleList([DecoderLayer(d_model, d_k, d_v, h, d_ff, dropout,
        ...
        self.max_len = max_len
        self.padding_idx = padding_idx
        self.pe_every_layer = pe_every_layer
    ...
    def forward(self, input, encoder_output, mask_encoder):
        ...
        mask_self_attention = mask_self_attention.gt(0)  # (b_s, 1, seq_len, seq_len)
        if self._is_stateful:
            self.running_mask_self_attention = torch.cat([self.running_mask_self_attention, mask_self_attention.byte()],-1)
            mask_self_attention = self.running_mask_self_attention
