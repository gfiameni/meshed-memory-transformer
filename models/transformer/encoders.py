...
class LinearEncoder(BaseEncoder):
    def __init__(self, N, padding_idx, d_in=2048, **kwargs):
        super(LinearEncoder, self).__init__(N, padding_idx, **kwargs)
        self.fc = nn.Linear(d_in, self.d_model)
        self.dropout = nn.Dropout(p=self.dropout)
        self.layer_norm = nn.LayerNorm(self.d_model)

    def forward(self, input, attention_weights=None):
        attention_mask = (torch.sum(input, -1) == self.padding_idx).unsqueeze(1).unsqueeze(
            1)  # (b_s, 1, 1, seq_len)

        out = F.relu(self.fc(input))
        out = self.dropout(out)
        out = self.layer_norm(out)
        return super(LinearEncoder, self).forward(out, attention_mask, attention_weights=attention_weights)
